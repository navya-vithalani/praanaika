...
function InsightCardView({ insight, dayLabel, disclaimerText }: { insight: any; dayLabel: string; disclaimerText: string }) {
  ...
  return (
    <Card className={`insight-panel insight-panel--${insight?.tier ?? 'direct'} insight-sources--${insight?.sources?.join('-') ?? 'self'}`} style={{ marginBottom: '16px', minWidth: '85vw', scrollSnapAlign: 'center', flexShrink: 0, color: '#fff' }}>
      ...
      <div className="insight-panel__body"><Mascot mood="happy" size={76} /><div><h2 style={{ color: '#fff' }}>{insight?.headline ?? 'I do not know your rhythm yet.'}</h2><p style={{ color: 'rgba(255,255,255,0.9)' }}>{insight?.body ?? 'Share a check-in and I will start learning your own baseline.'}</p></div></div>
      {insight?.tier === 'personal' && <span className="personal-badge">Just for you</span>}<button className="panel-link" style={{ color: '#fff' }} onClick={() => setExpanded(!expanded)}>Why I’m saying this <ArrowUpRight size={15} aria-hidden="true" /></button>{expanded && <div className="insight-evidence" style={{ color: 'rgba(255,255,255,0.8)' }}><strong>Here’s what I looked at:</strong>{(insight?.evidence ?? ['Your available check-ins and logs']).map((item: string) => <span key={item}>• {item}</span>)}<small>Observation, not a cause. {disclaimerText}</small></div>}
    </Card>
  );
}
...
export function YouScreen() {
  ...
  {summaryPreview && <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.8)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }} onClick={() => setSummaryPreview(false)}><div style={{ background: 'var(--color-background)', padding: '24px', borderRadius: '16px', maxHeight: '80vh', overflowY: 'auto', width: '100%', maxWidth: '500px' }} onClick={e => e.stopPropagation()}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}><h2 style={{ fontSize: '18px', color: 'var(--color-text)' }}>Medical Summary</h2><button className="icon-button" onClick={() => setSummaryPreview(false)}><X size={20} /></button></div><pre style={{ whiteSpace: 'pre-wrap', fontSize: '13px', lineHeight: 1.5, color: 'var(--color-text)', fontFamily: 'inherit' }}>{doctorSummaryContent}</pre></div></div>}
  </main>;
}
